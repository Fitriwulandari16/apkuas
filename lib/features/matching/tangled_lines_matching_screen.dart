...
class TangledLinesMatchingScreen extends ConsumerStatefulWidget {
  final int levelId;
  const TangledLinesMatchingScreen({super.key, this.levelId = 28});

  @override
  ConsumerState<TangledLinesMatchingScreen> createState() => _TangledLinesMatchingScreenState();
}

class _TangledLinesMatchingScreenState extends ConsumerState<TangledLinesMatchingScreen> {
  // Color palette definitions
  static const Color colOrange = Color(0xFFFFAA00); // 1
  static const Color colGreen = Color(0xFF4CAF50);  // 2
  static const Color colBrown = Color(0xFF8D6E63);  // 3
  static const Color colPurple = Color(0xFFAB47BC); // 4
  static const Color colYellow = Color(0xFFFFD54F); // 5

  // Map of number to correct target color
  ...
  // State: holds the color placed in each slot (1 to 5)
            ...
            Expanded(
              child: SingleChildScrollView(
                ...
                child: Padding(
                  ...
                  child: Column(
                    children: [
                      // Tangled Lines Canvas Box
                      _buildTangledCanvas(),

                      const SizedBox(height: 12),

                      // Target slots bar (1 to 5)
                      _buildTargetBar(),

                      const SizedBox(height: 16),

                      // Ember Warna / Color Dock
                      _buildColorDock(),
  ...
  Widget _buildTangledCanvas() {
    return AspectRatio(
      ...
      child: Container(
        ...
        decoration: BoxDecoration(
          color: const Color(0xFFEDE7F6), // Smooth purple container background
        ...
        child: LayoutBuilder(
          builder: (context, constraints) {
            ...
            // Coordinates mapping for start and end circle centers
            Offset getPos(double dx, double dy) => Offset(dx * size.width, dy * size.height);

            // Define node position map
            final nodePositions = {
              '1': getPos(0.50, 0.15),
              '2': getPos(0.18, 0.32),
              '3': getPos(0.32, 0.17),
              '4': getPos(0.68, 0.17),
              '5': getPos(0.18, 0.58),
              '3_col': getPos(0.85, 0.32), // Brown (visualized as Light Blue in photo)
              '4_col': getPos(0.85, 0.58), // Purple
              '5_col': getPos(0.32, 0.83), // Yellow
              '2_col': getPos(0.50, 0.83), // Green
              '1_col': getPos(0.68, 0.83), // Orange
            };

            return Stack(
              children: [
                // Custom Paint Tangled Dashed Lines
                Positioned.fill(
                  child: CustomPaint(
                    painter: TangledLinesPainter(
                      nodes: nodePositions,
                    ),
                  ),
                ),

                // Number nodes (interactive or visual start anchors)
          ...
          return Expanded(
            child: Column(
              children: [
                ...
                // Drag Target slot
                DragTarget<Color>(
                  onWillAcceptWithDetails: (details) => _placedColors[number] == null,
                  onAcceptWithDetails: (details) => _handleColorDrop(number, details.data),
                  builder: (context, candidateData, rejectedData) {
                    ...
                    return AnimatedContainer(
                      ...
                      height: 46,
                      decoration: BoxDecoration(
                        color: placedColor ?? (isHovered ? Colors.deepPurple.shade50 : Colors.white),
                        shape: BoxShape.circle,
                        border: Border.all(
                          color: placedColor != null
                              ? Colors.transparent
                              : (isHovered ? Colors.deepPurple : Colors.grey.shade300),
                          width: placedColor != null ? 0 : (isHovered ? 2.5 : 1.5),
                          style: placedColor != null ? BorderStyle.none : BorderStyle.solid,
                        ),
                        boxShadow: [
                          if (placedColor != null)
                            BoxShadow(
  ...
  Widget _buildColorBucket(Color color, {bool isShadow = false, bool isDragged = false}) {
...
class TangledLinesPainter extends CustomPainter {
  final Map<String, Offset> nodes;
  TangledLinesPainter({required this.nodes});

  @override
  void paint(Canvas canvas, Size size) {
    final Map<Color, Path> coloredPaths = {
      // 1 (top center) -> Orange (bottom right)
      Color(0xFFFFAA00): Path()
      ...
      // 2 (middle left) -> Green (bottom center)
      Color(0xFF4CAF50): Path()
      ...
      // 3 (top left) -> Brown (top right)
      Color(0xFF8D6E63): Path()
      ...
      // 4 (top right) -> Purple (middle right)
      Color(0xFFAB47BC): Path()
      ...
      // 5 (bottom left) -> Yellow (bottom left)
      Color(0xFFFFD54F): Path()
    ...
    // Draw the 5 tangled curves with a clean dotted/dashed stroke style
