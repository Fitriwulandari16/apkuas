...
class PatternShapeImitationScreen extends ConsumerStatefulWidget {
  final int levelId;
  const PatternShapeImitationScreen({super.key, this.levelId = 26});

  @override
  ConsumerState<PatternShapeImitationScreen> createState() => _PatternShapeImitationScreenState();
}

class ShapePatternChallenge {
  final int id;
  final Color color;
  final String title;
  final List<Set<int>> expectedLines;
  final List<int> vertexOrder; // Order of indices to draw the filled path
  List<Set<int>> drawnLines;
  bool isCompleted;

  ShapePatternChallenge({
    required this.id,
    required this.color,
    required this.title,
    required this.expectedLines,
    required this.vertexOrder,
  })  : drawnLines = [],
        isCompleted = false;
}

class _PatternShapeImitationScreenState extends ConsumerState<PatternShapeImitationScreen> {
  late List<ShapePatternChallenge> _challenges;
  ...
  static const Color colPurple = Color(0xFFAB47BC); // Baris 2: Ungu (Segitiga Siku Terbalik)
  static const Color colOrange = Color(0xFFFF9800); // Baris 3: Oranye (Segilima Rumah)
  static const Color colTeal = Color(0xFF26A69A);   // Baris 4: Tosca (Segilima Terbalik)
  ...
  void _initLevel() {
    _challenges = [
      // Baris 1: Segitiga Siku-Siku Besar
      ShapePatternChallenge(
        id: 0,
        color: colRed,
        title: 'Segitiga Siku-Siku',
        expectedLines: [
          {0, 9},  // Vertikal kiri (0-3-6-9)
          {9, 11}, // Horizontal bawah (9-10-11)
          {11, 0}, // Diagonal (11-0)
        ],
        vertexOrder: [0, 9, 11],
      ),
      // Baris 2: Segitiga Siku-Siku Terbalik
      ShapePatternChallenge(
        id: 1,
        color: colPurple,
        title: 'Segitiga Siku Terbalik',
        expectedLines: [
          {9, 11}, // Horizontal bawah (9-10-11)
          {11, 2}, // Vertikal kanan (11-8-5-2)
          {2, 9},  // Diagonal (2-9)
        ],
        vertexOrder: [9, 11, 2],
      ),
      // Baris 3: Bangun Segilima (Rumah)
      ShapePatternChallenge(
        id: 2,
        color: colOrange,
        title: 'Segilima (Rumah)',
        expectedLines: [
          {1, 3},  // Atap kiri (1-3)
          {3, 9},  // Dinding kiri (3-6-9)
          {9, 11}, // Lantai bawah (9-10-11)
          {11, 5}, // Dinding kanan (11-8-5)
          {5, 1},  // Atap kanan (5-1)
        ],
        vertexOrder: [1, 3, 9, 11, 5],
      ),
      // Baris 4: Bangun Segilima Terbalik
      ShapePatternChallenge(
        id: 3,
        color: colTeal,
        title: 'Segilima Terbalik',
        expectedLines: [
          {10, 6}, // Bawah kiri (10-6)
          {6, 0},  // Dinding kiri (6-3-0)
          {0, 2},  // Langit-langit (0-1-2)
          {2, 8},  // Dinding kanan (2-5-8)
          {8, 10}, // Bawah kanan (8-10)
        ],
        vertexOrder: [10, 6, 0, 2, 8],
      ),
    ];
  }
  ...
  Offset _getDotPosition(int index, Size size) {
    ...
    double spacingX = (size.width - 2 * paddingX) / 2;
    // Calculate paddingY to vertically center the 4 rows (4 rows = height of 3 spacings)
    double gridHeight = spacingX * 3;
    double paddingY = (size.height - gridHeight) / 2;

    int row = index ~/ 3;
    int col = index % 3;
    return Offset(paddingX + col * spacingX, paddingY + row * spacingX);
  }
  ...
  void _onLevelComplete() async {
    ...
    try {
      await UserService.updateProgress(26);
    } catch (e) {
      debugPrint('Cloud progress update failed for level 26: $e');
    }

    if (!mounted) return;
    CelebrationUtils.showCelebrationAndLevelUp(
      context: context,
      nextLevelId: 27, // Fallback for next stages
      title: 'Hebat, Kamu Juara!',
      message: 'Semua bentuk geometri berhasil ditiru dengan sempurna!',
    );
  }
                ...
                child: Padding(
                  ...
                  child: Column(
                    children: [
                      ...
                      const WavyDivider(color: colRed),
                      _buildRowChallenge(1),
                      const WavyDivider(color: colPurple),
                      _buildRowChallenge(2),
                      const WavyDivider(color: colOrange),
  ...
  Widget _buildInstruction() {
    return Container(
      ...
      child: Row(
        ...
        children: [
          const Icon(Icons.category_rounded, color: Colors.purple, size: 24),
                  ...
                  child: Container(
                    ...
                    child: LayoutBuilder(
                      builder: (context, constraints) {
                        ...
                        return Stack(
                          children: [
                            // Closed Path Fill Color
                            Positioned.fill(
                              child: CustomPaint(
                                painter: ShapeFillPainter(
                                  vertexOrder: challenge.vertexOrder,
                    ...
                    builder: (context, constraints) {
                      ...
                      return GestureDetector(
                        ...
                        child: AnimatedContainer(
                          ...
                          child: Stack(
                            children: [
                              // Closed Path Fill Color (when correct)
                              Positioned.fill(
                                child: CustomPaint(
                                  painter: ShapeFillPainter(
                                    vertexOrder: challenge.vertexOrder,
                              ...
                              ...List.generate(12, (dotIndex) {
                                ...
                                return Positioned(
                                  ...
                                  child: AnimatedContainer(
                                    ...
                                    width: isStartDot ? 18 : 14,
                                    height: isStartDot ? 18 : 14,
                                    decoration: BoxDecoration(
...
class _WavyPainter extends CustomPainter {
  ...
  @override
  bool shouldRepaint(covariant _WavyPainter oldDelegate) => false;
}
...
class ShapeFillPainter extends CustomPainter {
  ...
  @override
  void paint(Canvas canvas, Size size) {
    if (!isCompleted || vertexOrder.isEmpty) return;
    ...
    final path = Path();
    final firstPos = _getDotPosition(vertexOrder.first, size);
    path.moveTo(firstPos.dx, firstPos.dy);

    for (int i = 1; i < vertexOrder.length; i++) {
      final pos = _getDotPosition(vertexOrder[i], size);
