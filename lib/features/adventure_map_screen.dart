                        ...
                        children: [
                          for (var entry in levelPositions.entries)
                            Positioned(
                              ...
                              top:
                                  totalHeight - entry.value.dy - (tileSize / 2),
                              child: _LevelTile(
                                level: entry.key,
                                isUnlocked: entry.key <= unlockedLevel,
                                isCurrent: entry.key == unlockedLevel,
                              ),
                            ),
                          if (levelPositions.containsKey(unlockedLevel))
                            AnimatedPositioned(
                              ...
                              left:
                                  centerX +
                                  levelPositions[unlockedLevel]!.dx -
                                  (tileSize / 2) +
                                  8,
                              top:
                                  totalHeight -
                                  levelPositions[unlockedLevel]!.dy -
                                  (tileSize / 2) -
                                  18,
  ...
  Widget build(BuildContext context) {
    return GestureDetector(
      ...
      child: AnimatedContainer(
        ...
        width: AdventureMapScreen.tileSize,
        height: AdventureMapScreen.tileSize,
        decoration: BoxDecoration(
          color: isCurrent
              ? const Color(0xFF4A68B1)
              : (isUnlocked
                    ? Colors.yellow.withOpacity(0.9)
                    : const Color(0xFF1E2F5A)),
          borderRadius: BorderRadius.circular(8),
          border: Border.all(
            color: isCurrent
                ? Colors.white
                : (isUnlocked ? Colors.orange.shade300 : Colors.transparent),
            width: isCurrent ? 2 : 1,
          ),
          boxShadow: isCurrent
        ...
        child: Text(
          ...
          style: TextStyle(
            color: isCurrent
                ? Colors.white
                : (isUnlocked ? Colors.brown.shade700 : Colors.white24),
            fontSize: 12,
      ...
      child: Container(
        ...
        decoration: BoxDecoration(
          gradient: LinearGradient(
            ...
            colors: [
              Colors.transparent,
              const Color(0xFF2E4482).withOpacity(0.9),
              const Color(0xFF2E4482),
            ],
          ),
        ),
        ...
        child: ElevatedButton(
          onPressed: () => Navigator.push(
            context,
            MaterialPageRoute(
              builder: (context) => LevelResolver.buildLevel(unlockedLevel),
            ),
          ),
